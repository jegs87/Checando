...
class MY_Model extends CI_Model
{
    ...
    /**
    * Funcion getListMultiselect devuelve la consulta a partir de id´s numéricos IN (1,2,3, etc...)
    * Copiada de taquillometro
    * Added by jupazave
    *
    *
    */
    function getListMultiselect($id){
        if ($id != null){
        	$id = explode("multiselect-all,",$id);
            if (count($id)==2){
                $query = $this->db->query("
            	...
            	/*$this->db->select($this->selectDefault,false);
            	$this->db->where_in($this->alias.'.id',$id[0]);
            	$query = $this->db->get($this->table.' AS '.$this->alias); 
            	return $query;*/
        	}
        	if ($query->num_rows() > 0) {
